<template>
  <contain :sping="sping" class="preview-container">
    <div slot="top-bar" class="top">
      <span>
        <span>字体大小:</span>
        <Select v-model="fontSize" size="small" style="width: 66px">
          <Option v-for="size in fontSizes" :value="size" :key="size">{{ size }}px</Option>
        </Select>
        <Divider type="vertical" />
        <span>字体类型:</span>
        <Select v-model="fontType" size="small" style="width: 74px">
          <Option v-for="(family, index) in fontTypes" :value="family" :key="family">类型{{ index + 1 }}</Option>
        </Select>
      </span>
      <Space>
        <Button type="info" @click="modifyBlog">编辑</Button>
    ...
    <div class="container-box">
      ...
      <div class="content" :style="{ 'font-size': fontSize + 'px', 'font-family': fontType }" v-html="blog.content"></div>
    </div>
  </contain>
</template>
...
export default {
  ...
  methods: {
    getBlog() {
      ...
      getSingleBlog({ id: blogId }).then(res => {
        if (res.status === 200) {
          this.sping = false
          this.blog = res.data
        }
