<template>
    <contain :showSearch="false">
        <Row :wrap="false" class="container-row">
            <Col class="left-list" flex="350px">
            <sping v-if="listLoad">
            </sping>
            <Button type="primary" icon="md-add" @click="onAddBlog">新增</Button>
            <Input style="margin-top: 12px;" search placeholder="请搜索" @on-enter="e => getBlogList(e.target.value)"></Input>
            <List>
                <ListItem v-for="(it, index) in blogList" :key="index" :class="{ 'selected-item': selectedId === it.id }">
                    <ListItemMeta @click.native="() => onClickBlogItem(it)" :title="it.title">
                        <span slot="avatar">
                            <svg t="1609063644144" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                xmlns="http://www.w3.org/2000/svg" p-id="9707" width="32" height="32">
                                <path d="M512 512m-512 0a512 512 0 1 0 1024 0 512 512 0 1 0-1024 0Z" fill="#BDC3C7"
                                    p-id="9708" />
                                <path
                                    ...
                                    fill="#6B7082" p-id="9709" />
                                <path d="M301.169778 439.068444h421.546666V584.817778H301.169778z" fill="#FABD6B"
                                    p-id="9710" />
                                <path
                                    ...
                                    fill="#464C63" p-id="9711" />
                                <path d="M642.844444 250.311111v523.377778h-58.140444V250.311111" fill="#E1E1E6"
                                    p-id="9712" />
                                <path
...
import { getBlogList, getSingleBlog, modifyBlog, addBlog, deleteBlog } from '@/service';
...
export default {
    ...
    methods: {
        ...
        saveBlog: debounce(function () {
            modifyBlog(this.selectedBlog).then(res => {
                if (res.status === 200) {
