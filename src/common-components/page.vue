<template>
  <contain class="page" :sping="sping" :isHaveData="isHaveData" :search="getBlogList">
    <Breadcrumb slot="breadcrumb" :style="{ margin: '16px 0' }">
      <BreadcrumbItem v-for="(path, index) in $route.path.slice(1, $route.path.length).split('/')" :key="index">
        {{ path.toUpperCase() }}</BreadcrumbItem>
      <Button type="primary" class="create-btn" @click="addBlog" size="small">创建博客</Button>
    </Breadcrumb>
    <List>
      <ListItem v-for="(blogObj, index) in blog_arr" :key="index">
        <ListItemMeta :description="blogObj['description']">
          <span slot="avatar">
            <svg t="1609063644144" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
              p-id="9707" width="32" height="32">
              <path d="M512 512m-512 0a512 512 0 1 0 1024 0 512 512 0 1 0-1024 0Z" fill="#BDC3C7" p-id="9708"></path>
              <path
                ...
                fill="#6B7082" p-id="9709"></path>
              <path d="M301.169778 439.068444h421.546666V584.817778H301.169778z" fill="#FABD6B" p-id="9710"></path>
              <path
                ...
                fill="#464C63" p-id="9711"></path>
              <path d="M642.844444 250.311111v523.377778h-58.140444V250.311111" fill="#E1E1E6" p-id="9712"></path>
              <path
        ...
        <div class="like-finger" @click="() => changeLikeNum(blogObj, index)">
          <span class="svg-box">
            <svg t="1609058419482" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
              p-id="2742" width="100%" height="100%">
              <path
        ...
        <template slot="action">
          ...
          <li>
            <a @click="previewBlog(blogObj['id'])">预览</a>
          </li>
          <li>
  ...
  methods: {
    ...
    changeLikeNum(blogObj, index) {
      ...
      if (this.like) {
        ...
        this.blog_arr[index].like = this.blog_arr[index].like - 1 < 0 ? 0 : this.blog_arr[index].like - 1
      }
      ...
      changeLikeNum(params).then(res => { }).catch(err => {
        this.$Message.error({
          background: true,
          content: err
        })
